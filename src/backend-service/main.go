...
func main() {
	...
	if err := mysql.InitGormForFrontService(settings.Conf.MySQLConfig); err != nil {
		fmt.Printf("init mysql failed, err:%v\n", err)
		return
	}
	defer mysql.Close()
	if err := v1.InitGoodsRPCClient(settings.Conf.GoodsConfig); err != nil {
		fmt.Printf("init goods gRPC failed, err:%v\n", err)
		return
	}
	if err := v1.InitOrderRPCClient(settings.Conf.OrderConfig); err != nil {
		fmt.Printf("init order gRPC failed, err:%v\n", err)
		return
	}
	// 4. 初始化Redis连接
