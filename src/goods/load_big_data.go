...
// 批量插入数据
func BatchSave(db *gorm.DB, data []*models.Goods) error {
	var buffer bytes.Buffer
	sql := "insert into `tb_goods` (`goods_uuid`,`goods_type_id`,`primary_type`,`secondary_type`,`price`, `title`, `subtitle`, `img`, `imgs`, `stock`) values"
	if _, err := buffer.WriteString(sql); err != nil {
		return err
	}
	for i, e := range data {
		if i == len(data)-1 {
			buffer.WriteString(fmt.Sprintf("('%s','%d','%s','%s','%d','%s','%s','%s','%s','%d');",
				e.GoodsUuid, e.GoodsTypeId, e.PrimaryType, e.SecondaryType, e.Price, e.Title, e.Subtitle, e.Img, e.Imgs, e.Stock))
		} else {
			buffer.WriteString(fmt.Sprintf("('%s','%d','%s','%s','%d','%s','%s','%s','%s','%d'),",
				e.GoodsUuid, e.GoodsTypeId, e.PrimaryType, e.SecondaryType, e.Price, e.Title, e.Subtitle, e.Img, e.Imgs, e.Stock))
		}
